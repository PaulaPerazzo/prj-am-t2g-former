...
def gmean_score(y_true, y_pred, eps=1e-9):
    """
    G-MEAN para classificação multi-classe.
    """
...
def objective(trial, dataset_name):
    ...
    for fold_idx, (train_idx, valid_idx) in enumerate(kf.split(X_all_num)):
        X_train_num, X_valid_num = X_all_num[train_idx], X_all_num[valid_idx]
        ...
        for epoch in range(200):
            ...
            # G-MEAN no lugar da acurácia
            gmean = gmean_score(y_all_fold, preds_all)
...
if __name__ == "__main__":
    ...
    print(f"total time spent on unbalanced datasets: {total_time}")
