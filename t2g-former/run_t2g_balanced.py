...
from tune_t2g_for_balanced_data import (
    DEVICE,
    seed_everything,
    load_dataset,
    preprocess,
...
def train_final_model_for_dataset(dataset_name):
    print(f"\n===== balanced dataset: {dataset_name} =====")

    # 1. optuna hyperparams
    best_params, best_cv_metric = load_best_params(dataset_name)
    print("Best metric (CV Optuna):", best_cv_metric)
    ...
    for epoch in range(200):
        ...
        acc_val = accuracy_score(y_all_val, preds_all)
        ...
        if acc_val > best_val_acc:
            ...
            if no_improve >= patience:
                print(f"Early stopping at epoch {epoch+1}, best ACC (val) = {best_val_acc:.4f}")
                break
    ...
    model_path = os.path.join(MODELS_DIR, f"{dataset_name}_t2gformer_balanced.pt")
